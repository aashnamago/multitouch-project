...
public class MultiTouchMenu : MonoBehaviour {
	...
	private Dictionary<GameObject, List<float>> beingTouched = new Dictionary<GameObject, List<float>>(); // maps an object that is being touched to a list of fingerIds (fingers touching it)
	private Dictionary<GameObject, float> originalY = new Dictionary<GameObject, float>(); // maps an object to its original y-coordinates
	private Dictionary<GameObject, Vector3> offsets = new Dictionary<GameObject, Vector3>(); // maps an object to its offset from the fingers moving it (right now, only moves if one finger touching it) 
