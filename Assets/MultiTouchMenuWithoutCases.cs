...
//			touchHistory history = new touchHistory();
//			if (history.fingerIds.Count == 3 && history.numTouches == 1 && history.began == 0 && history.ended == 0) {
//				if (!beingTouched.ContainsKey (key)) {
//					Debug.Log("linear expansion");
//					generateLine(key.transform);
//				}
//			}
//		}
...
//			} 
	...
	void generateLine(Transform buttonTransform) {
		float cylinderScale = buttonTransform.localScale.x;
		GameObject firstInLine = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
		firstInLine.name = "Submenu";
		firstInLine.transform.localScale = new Vector3(cylinderScale, originalY, cylinderScale);
		firstInLine.transform.position = buttonTransform.position + new Vector3(10f, 10f, 10f);
		firstInLine.renderer.material.color = Color.black;
		firstInLine.layer = 8;
	}
